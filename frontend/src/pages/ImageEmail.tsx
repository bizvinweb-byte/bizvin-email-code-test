    ...
    <div style={{ position: "relative" }}>
      ...
      <input
        ...
        onFocus={(e) => {
          ...
          e.target.style.boxShadow = "0 0 0 3px rgba(255,120,0,0.1)";
...
const ImageEmail = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const emailsArray = formData.toEmails
      .split(/[\s,]+/)
      .filter((e) => e.trim())
      .map((e) => e.trim());
    ...
    if (formData.emailConfigIds.length === 0) {
      setStatus({
        type: "error",
        message: "Please select at least one Sender Account.",
      });
      return;
    }
    if (isScheduled && !scheduledAt) {
      setStatus({
        type: "error",
        message: "Please provide a schedule date and time.",
      });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      setStatus({
        type: "success",
        message: isScheduled
          ? `✓ Campaign scheduled for ${emailsArray.length} recipient(s)!`
          : `✓ Campaign sent to ${emailsArray.length} recipient(s)!`,
      });
      setFormData({
        fromName: "",
        fromEmail: "",
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "rgba(255,200,120,0.6)" }}>
            Send an image or GIF as email campaign
          </p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <div style={labelStyle}>
                  <Mail size={12} />
                  Sender Accounts (Select one or more for rotation)
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {emailConfigs.map((c: any) => (
                      <label
                        ...
                        style={{
                          background: formData.emailConfigIds.includes(c._id)
                            ? "rgba(255,107,0,0.15)"
                            : "rgba(255,255,255,0.03)",
                          border: formData.emailConfigIds.includes(c._id)
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newIds = e.target.checked
                              ...
                              : formData.emailConfigIds.filter(
                                  (id) => id !== c._id,
                                );
                            setFormData({
                              ...formData,
                              emailConfigIds: newIds,
                            });
                          }}
                        />
              ...
              />
              <Field
                label="From Email (Custom Override)"
                icon={Mail}
                type="email"
                name="fromEmail"
                value={formData.fromEmail}
                onChange={handleChange}
                placeholder="custom@company.com (Optional)"
              />
              <Field
              ...
              <div className="md:col-span-2">
                <Field
                  label="To Emails (comma or space separated)"
                  icon={Mail}
                  type="text"
                  name="toEmails"
                  required
              ...
              <div className="md:col-span-2">
                <div className="flex justify-between items-end mb-2">
                  ...
                  <span className="text-[10px] text-slate-500 bg-white/5 px-2 py-1 rounded-md border border-white/5">
                    Example: &lt;a href="https://google.com"&gt;Click
                    Here&lt;/a&gt;
                  </span>
                </div>
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="This email was sent to you because... <a href='https://example.com'>Unsubscribe</a>"
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 transition-all min-h-[100px]"
                />
              </div>
              <SelectField
                label="Disclaimer Position"
                icon={AlignCenter}
                name="disclaimerPosition"
                value={formData.disclaimerPosition}
                onChange={handleChange}
              >
                <option value="" disabled style={{ background: "#1a1035" }}>
                  — Select Position —
                </option>
                <option value="Bottom" style={{ background: "#1a1035" }}>
                  Bottom
                </option>
                <option value="Top" style={{ background: "#1a1035" }}>
                  Top
                </option>
              </SelectField>
              <div className="md:col-span-2">
                <Field
                  label="Link on Image (URL)"
                  icon={Link}
                  type="url"
                  name="linkOnImage"
                  value={formData.linkOnImage}
                  onChange={handleChange}
            ...
            {imageFiles.length === 0 && (
              <p className="text-xs" style={{ color: "#FB8500" }}>
                ...
                onMouseEnter={(e) => {
                  if (!loading)
                    (e.currentTarget as HTMLButtonElement).style.boxShadow =
                      "0 10px 30px rgba(255,107,0,0.55)";
                }}
                onMouseLeave={(e) => {
                  (e.currentTarget as HTMLButtonElement).style.boxShadow =
